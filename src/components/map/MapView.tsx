...
export function MapView({ activities, onActivityClick, selectedActivityId, searchedLocation, onCreateAtLocation }: { activities: any[], onActivityClick?: (act: any) => void, selectedActivityId?: number, searchedLocation?: any, onCreateAtLocation?: (loc: any) => void }) {
  ...
  useEffect(() => {
    ...
    if (mapRef.current) {
      ...
      const loadAndRenderCluster = async () => {
        if (!(L as any).markerClusterGroup) {
          ...
          if (!document.getElementById('markercluster-css')) {
            ...
            ['MarkerCluster.css', 'MarkerCluster.Default.css'].forEach((file, i) => {
              ...
              link.id = i === 0 ? 'markercluster-css' : 'markercluster-default-css';
              document.head.appendChild(link);
            });
          }
          // Use script tag to load the UMD bundle instead of import()
          await new Promise<void>((resolve) => {
            ...
            script.onerror = () => resolve(); // fail silently, cluster just won't group
